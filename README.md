# Jev Lab · 学习 TypeSafe Jev 的 12 个可视化场景
...
本仓库是一套学习材料 + 可运行的 Web 应用：8 个基础场景 + 4 个行业判断场景（教育 / 医疗 / 金融 / 能源）按学习路径排列，每个场景都展示**发给 Jev 的原始问题、返回的概率、延迟与费用**，以及"如果用 LLM 做同样的事"的成本基线；其中 4 个场景把 Jev 与 Claude（经 AWS Bedrock）组合使用。
