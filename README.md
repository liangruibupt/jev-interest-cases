...
进入 `/c5`，把英文画面描述变成可播放的器乐：一次 Jev 请求给出音色、能量、明亮度、
密度、张力、鼓点与静音判断，浏览器用确定性的四小节音序和 Web Audio 合成声音。
手动混音、变奏和 WAV 导出不再调用模型；初始音色明确标记为手动预置，点击播放才发声。

`npm run smoke -- c5` 评估四个公开预置并保存可回放缓存。自定义描述只读缓存，不落盘。
新端口预览可用 `API_PROXY_TARGET=http://127.0.0.1:8788` 指定 Vite 代理。
详见 [`docs/scenarios/C5.md`](docs/scenarios/C5.md)。成本只对比参数判断，不与音频生成模型虚比。
