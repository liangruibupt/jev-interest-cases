...
| 3 | **A4 一致性与校准对比** | 用自己的数据复现 Jev vs Claude 的稳定性 / 延迟 / 成本 | 实测 |
| 4 | **B1 护栏 + 模型路由** | Jev 作为 Claude 前面 1% 成本的分流器与护栏 | 分类步骤 ≈99%；端到端 55–65% |
| 5 | **B2 引用核验** | 生成者与检验者分离，用 LLM 千分之几的成本核查引用 | 核验步骤 ≈98–99%；端到端 ≈33% |
| 6 | **A3 文档逐行语义搜索** | 无 embedding 的检索；Choice 概率和为 1 必须配存在性 Noul | 对 LLM ≈98%；对向量检索不省钱 |
| 7 | **B3 RAG 段落守门人** | 证据筛选与提示注入检测；让 LLM 敢于反驳错误前提 | 守门步骤 ≈99%；端到端 ≈40% |
| 8 | **B4 智能家居助手** | 带概率的 function calling；数字留给正则；高风险动作更高门限 | 纯指令 ≈98%；混合流量 83–93% |
