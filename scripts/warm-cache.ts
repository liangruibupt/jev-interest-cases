...
import { A1_PRESETS, A2_QUESTIONS, A3_PRESET_QUERIES, B1_QUESTIONS, B2_QUESTIONS, B3_PRESET_QUERIES, B3_QUESTIONS, C1_QUESTIONS, C2_QUESTIONS, C3_QUESTIONS, C4_QUESTIONS, CORPUS, ESSAYS, EXAMPLE_REQUESTS, FILINGS, INITIAL_HOME, PATIENT_MESSAGES, ROOMS, VPP_NOTICES, VPP_SITES, CANNED_CITATIONS, GUARDRAIL_MESSAGES, RFC_SECTIONS, TICKETS, buildClaimState, buildDocumentState, buildFindQuestions, bm25Search, buildB4Questions, buildEssayState, buildFilingState, buildIndex, buildMessageState, buildPairState, buildPassageState, buildPatientState, buildRequestState, buildTicketState, passageById, stringStage } from "../shared/src/index";
...
const c4Pairs = VPP_NOTICES.flatMap((n) => VPP_SITES.map((s) => ({ n, s })));
const c4 = await Promise.all(c4Pairs.map(({ n, s }) => askJev({ scenario: "c4", state: buildPairState(n, s), questions: C4_QUESTIONS }, { cache: "read-write" })));
c4.forEach((o, i) => note(`c4/${c4Pairs[i]!.n.id}:${c4Pairs[i]!.s.id}`, o.trace));
