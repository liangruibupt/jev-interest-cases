...
export async function c2(): Promise<void> {
  ...
  outcomes.forEach((o, i) => {
    ...
    rows.push({
      ...
      chest: d.redFlags.find((f) => f.id === "red_flag_chest_pain")?.value.toFixed(2),
      breath: d.redFlags.find((f) => f.id === "red_flag_breathing")?.value.toFixed(2),
      selfHarm: d.redFlags.find((f) => f.id === "red_flag_self_harm")?.value.toFixed(2),
      stroke: d.redFlags.find((f) => f.id === "red_flag_stroke_or_bleeding")?.value.toFixed(2),
